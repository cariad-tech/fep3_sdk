...
class AsyncReceiver : public ISimulationBus::IDataReceiver
{
public:
    ...
    {
        int buffer = 0;
        auto raw_buffer = base::RawMemoryStandardType<int>(buffer);
...
public:
    ...
    //Begin(SetAsyncWaitSet)
    fep3::Result load() override
    {
        auto* conf_service = getComponents()->getComponent<IConfigurationService>();
        if (conf_service)
        {
    ...
    {
        auto* sim_bus = getComponents()->getComponent<ISimulationBus>();
        if (!sim_bus)
        {
...
int main(int argc, const char* argv[])
{
    ...
    auto demo_participant = core::createParticipant<core::ElementFactory<ReceiverElement>>(
